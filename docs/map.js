...
async function boot(){
  recap=await (await fetch("recap.json")).json();
  const q=Number(new URLSearchParams(location.search).get("week"));
  selected = q || recap.completed[recap.completed.length-1] || 1;
  ...
  recap.spine.forEach(function(s){
    ...
    d.title=s.title;
    d.addEventListener("click", function(){ show(s.n); history.replaceState(null,"","map.html?week="+s.n); });
    rail.appendChild(d);
  });
