...
assert.ok(indexHtml.indexOf("בְּנֵי הַבְּרִית") !== -1, "Hebrew title stays");
assert.ok(indexHtml.indexOf("This week") !== -1);
assert.ok(indexHtml.indexOf("Week 2") !== -1);
assert.ok(indexHtml.indexOf("sons of God") !== -1);
assert.ok(indexHtml.indexOf("Nephilim") !== -1);
assert.ok(indexHtml.indexOf("flood") !== -1 || indexHtml.indexOf("Flood") !== -1);
assert.ok(indexHtml.indexOf("Noah") !== -1);
assert.ok(indexHtml.indexOf("week.html?week=2") !== -1, "gold door to week 2");
assert.ok(indexHtml.indexOf("tap to read") === -1, "do not say tap to read on the next page");
assert.ok(indexHtml.indexOf("next page") === -1);
assert.ok(indexHtml.indexOf("Last week") !== -1);
assert.ok(indexHtml.indexOf("Yahweh alone creates") !== -1);
assert.ok(indexHtml.indexOf("council") !== -1);
assert.ok(indexHtml.indexOf("vocation") !== -1);
assert.ok(indexHtml.indexOf("nachash") !== -1);
assert.ok(indexHtml.indexOf("Genesis 3:15") !== -1);
assert.ok(indexHtml.indexOf("week.html?week=1") !== -1);
assert.ok(indexHtml.indexOf("job.html") !== -1, "optional Job door");
assert.ok(indexHtml.indexOf("Twelve stations") === -1);
assert.ok(indexHtml.indexOf("Story so far") === -1);
assert.ok(indexHtml.indexOf("map.html") === -1, "no Map on student home");
assert.ok(indexHtml.indexOf("class=\"card\"") === -1, "home is not cream homework cards");
assert.ok(indexHtml.indexOf("ancient-texts-app") === -1);
assert.ok(/class="cta"/.test(indexHtml), "one gold primary CTA");
assert.ok(indexHtml.indexOf("class=\"beats\"") !== -1, "last week uses headed beats");
assert.ok(indexHtml.indexOf("home-foot") !== -1, "tight Hebrew footer");
assert.ok((indexHtml.match(/class="hero"/g) || []).length === 1);
assert.ok(indexHtml.indexOf("home-nav") !== -1);
assert.ok(!/map\.html/.test(indexHtml));
...
assert.ok(weekPage.indexOf("week-window.js") !== -1);
